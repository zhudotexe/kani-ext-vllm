...
class HTTPTokenizerCompat:
    ...
    def _request_detokenize_str(self, tokens: list[int], **_):
        payload = {
            "model": self.model_id,
            "tokens": tokens,
        }
        log.debug(f"Detokenize: {payload!r}")
        resp = self.http.post("/detokenize", json=payload)
        resp.raise_for_status()
        data = resp.json()
        return data
    ...
    def apply_chat_template(self, messages: list[dict], tokenize: bool = True, **kwargs):
        data = self._request_tokenize_msg(messages, **kwargs)
        tokens = data["tokens"]
        if tokenize:
            return tokens
        detok_resp = self._request_detokenize_str(tokens)
        return detok_resp["prompt"]
